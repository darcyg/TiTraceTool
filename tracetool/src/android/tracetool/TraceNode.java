...
public class TraceNode extends TraceToSend
{
   ...
   {
      if (generateUniqueId)
         this.id = new UID().toString(); // java.rmi.server. // else : no more reset to empty string if generateUniqueId is false
   ...
   public TraceNode setFontDetail(final int colId, final boolean bold, final boolean italic, final int color)
   {
   ...
   public TraceNode setFontDetail(final int colId, final boolean bold, final boolean italic, final int color, final int size)
   {
   ...
   public TraceNode setFontDetail(final int colId, final boolean bold, final boolean italic, final int color, final int size, final String fontName)
   {
      if (!this.enabled)
         return this;
      ...
      if (color == -1)
         colorValue = -1;
          ...
          colorValue = (Color.blue(color)   << 16)   // This is the same as saying color & 0xFF
