...
public abstract class TraceToSend extends TraceNodeBase
{
    ...
    */
   protected ArrayList<String> prepareNewNode(final String leftMsg, final String newId)
   {
   ...
   public TraceNode send(final String leftMsg)
   {
      ...
      try
      {
         ...
         ArrayList<String> commandList = prepareNewNode(leftMsg, result.id);
   ...
   protected NodeContext getLastContext()
   {
      ...
      synchronized (cList)
      {
         for (Iterator<NodeContext> iter = cList.iterator(); iter.hasNext();)
         {
   ...
   {
      indent(leftMsg, "", -1, false); //$NON-NLS-1$
   }
    ...
    */
   public void indent(final String leftMsg, final String rightMsg, final int backGroundColor)
   {
    ...
    */
   public void indent(final String leftMsg, final String rightMsg, final int backGroundColor, final boolean isEnter)
   {
      if (!this.enabled)
         return;
      ...
      newContext.nodeId = new UID().toString();  // java.rmi.server.
      ArrayList<String> commandList = new ArrayList<String>();
      ...
      {
    	  int colorValue = (Color.blue(backGroundColor)   << 16)   // This is the same as saying color & 0xFF
                         | (Color.green(backGroundColor)  << 8)    // This is the same as saying (color >> 8) & 0xFF
                         | (Color.red(backGroundColor)    << 0);   // This is the same as saying (color >> 16) & 0xFF
   ...
   public void unIndent(final String leftMsg)
   {
      ...
      unIndent(leftMsg, "", -1, false); //$NON-NLS-1$
   }
    ...
    */
   public void unIndent(final String leftMsg, final String rightMsg, final int backGroundColor)
   {
      if (!this.enabled)
         return;
    ...
    */
   public void unIndent(final String leftMsg, final String rightMsg, final int backGroundColor, final boolean isExit)
   {
      if (!this.enabled)
         return;
      ...
      {
         String nodeId = new UID().toString(); // then give new ID

         ArrayList<String> commandList = prepareNewNode(leftMsg, nodeId);
   ...
   {
      indent("Enter " + leftMsg, "", -1, true); //$NON-NLS-1$ //$NON-NLS-2$
   }
   ...
   {
      indent("Enter " + leftMsg, rightMsg, -1, true); //$NON-NLS-1$
   }
    ...
    * @param backGroundColor BackGround Color
    */
   public void enterMethod(final String leftMsg, final String rightMsg, final int backGroundColor)
   {
   ...
   {
      unIndent("Exit", "", -1, true); //$NON-NLS-1$//$NON-NLS-2$
   }
   ...
   {
      unIndent("Exit " + leftMsg, "", -1, true);//$NON-NLS-1$//$NON-NLS-2$
   }
   ...
   {
      unIndent("Exit " + leftMsg, rightMsg, -1, true);//$NON-NLS-1$
   }
    ...
    * @param backGroundColor BackGround Color
    */
   public void exitMethod(final String leftMsg, final String rightMsg, final int backGroundColor)
   {
   ...
   public TraceNode sendValue(final String leftMsg, final Object objToSend, final boolean sendPrivate, final int maxLevel)
   {
      ...
      {
         Class<? extends Object> objClass = objToSend.getClass();
    ...
    * @return a Trace node
    */
   public TraceNode sendType(final String leftMsg, final Class<? extends Object> oType)
   {
    ...
    * @return a Trace node
    */
   public TraceNode sendType(final String leftMsg, final Class<? extends Object> oType, final int flags)
   {
      if (!this.enabled)
         return new TraceNode(this);
   ...
   */
   ...
   public TraceNode sendBackgroundColor(final String leftMsg, final int color)
  {
      if (!this.enabled)
         return new TraceNode(this);
    ...
    ArrayList<String> commandList = prepareNewNode(leftMsg, result.id);
    int colorValue = (Color.blue(color)   << 16)   // This is the same as saying color & 0xFF
                   | (Color.green(color)  << 8)    // This is the same as saying (color >> 8) & 0xFF
                   | (Color.red(color)    << 0);   // This is the same as saying (color >> 16) & 0xFF
 ...
 public TraceNode sendBackgroundColor(final String leftMsg, final int color, final int colId)
 {
    if (!this.enabled)
       return new TraceNode(this);
